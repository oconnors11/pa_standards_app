...
describe('Tier 1: Multi-Subject Graph Indexing & In-Memory Storage (Feature 1)', () => {
  test('T1.1.1: Complete Dataset Indexing — Indexes all 2,195 standards', async () => {
    const engine = await getEngine();
    const all = engine.getAllStandards();
    assert.strictEqual(all.length, 2195, 'Store must index exactly 2,195 standards');
  }, { tier: 1 });
  ...
  test('T1.1.2: Subject Census Verification — Accurately indexes all 5 PA subjects', async () => {
    ...
    assert.strictEqual(math.length, 517, 'Mathematics standards count must be 517');
    assert.strictEqual(ela.length, 1045, 'English Language Arts standards count must be 1045');
  ...
  test('T1.1.3: Primary Key O(1) Lookup — Resolves standards by exact code across subjects', async () => {
    const engine = await getEngine();
    const mathStd = engine.getStandardByCode('CCSS.MATH.CONTENT.3.NF.A.1');
    assert.ok(mathStd, 'Resolves Grade 3 Math standard CCSS.MATH.CONTENT.3.NF.A.1');
    assert.strictEqual(mathStd.code, 'CCSS.MATH.CONTENT.3.NF.A.1');
  ...
  test('T1.1.6: Store Immutability Check — Protects in-memory store from mutation', async () => {
    const engine = await getEngine();
    const s1 = engine.getStandardByCode('CCSS.MATH.CONTENT.3.NF.A.1');
    s1.tempMutationFlag = true;

    const s2 = engine.getStandardByCode('CCSS.MATH.CONTENT.3.NF.A.1');
    const all = engine.getAllStandards();
...
describe('Tier 1: 5-Tier Relationship Resolution Engine (Feature 2)', () => {
  ...
  test('T1.2.6: Graph Result Structural Integrity — Conforms to full interface contract', async () => {
    const engine = await getEngine();
    const result = engine.getCoherenceGraph('CCSS.ELA-LITERACY.RI.3.1');
...
describe('Tier 1: Search & Cascading Filter Selectors (Feature 3)', () => {
  test('T1.3.1: Exact Standard Code Query — Exact match ranks first', async () => {
    const engine = await getEngine();
    const results = engine.searchStandards('CCSS.MATH.CONTENT.3.NF.A.1');
    assert.ok(results.length > 0);
    assert.strictEqual(results[0].code, 'CCSS.MATH.CONTENT.3.NF.A.1');
  }, { tier: 1 });

  test('T1.3.2: Prefix / Partial Code Query — Matches standards with code prefix', async () => {
    const engine = await getEngine();
    const results = engine.searchStandards('CCSS.MATH');
    assert.ok(results.length > 0);
    assert.ok(results.some(s => s.code.startsWith('CCSS.MATH')));
  }, { tier: 1 });
...
describe('Tier 1: Interactive Breadcrumb Navigation Stack (Feature 4)', () => {
  test('T1.4.1: Initial Stack Initialization — Creates root breadcrumb', async () => {
    const engine = await getEngine();
    const trail = engine.addBreadcrumb([], 'CCSS.MATH.K.A.1');
    assert.deepStrictEqual(trail, ['CCSS.MATH.K.A.1']);
  }, { tier: 1 });

  test('T1.4.2: Linear Forward Traversal — Appends next step progression', async () => {
    const engine = await getEngine();
    const trail = engine.addBreadcrumb(['CCSS.MATH.K.A.1'], 'CCSS.MATH.1.B.1');
    assert.deepStrictEqual(trail, ['CCSS.MATH.K.A.1', 'CCSS.MATH.1.B.1']);
  }, { tier: 1 });

  test('T1.4.3: Multi-Step Progression Trail — Preserves multi-step vertical path', async () => {
    const engine = await getEngine();
    let trail = ['CCSS.MATH.K.A.1', 'CCSS.MATH.1.B.1'];
    trail = engine.addBreadcrumb(trail, 'CCSS.MATH.2.B.2');
    assert.deepStrictEqual(trail, ['CCSS.MATH.K.A.1', 'CCSS.MATH.1.B.1', 'CCSS.MATH.2.B.2']);
  }, { tier: 1 });
...
describe('Tier 1: Coherence Node Schema & Graph Interface Contracts (Feature 6)', () => {
  ...
  test('T1.6.6: 5-Subject Graph Feasibility — Generates valid graphs across all 5 subjects', async () => {
    const engine = await getEngine();
    const mathG = engine.getCoherenceGraph('CCSS.MATH.CONTENT.3.NF.A.1');
    const elaG = engine.getCoherenceGraph('CCSS.ELA-LITERACY.RI.3.1');
